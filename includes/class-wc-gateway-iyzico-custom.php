 ...
 * text domain 'iyzico-payment-gateway'. Kaynak dil Türkçe (iyzico zaten
...
class WC_Gateway_Iyzico_Custom extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = __('iyzico (Custom / 3D Secure)', 'iyzico-payment-gateway');
        $this->method_description = __('iyzico Checkout Form ile 3D Secure zorunlu ödeme. Kart verisi sitede tutulmaz.', 'iyzico-payment-gateway');
    ...
    private function get_media_picker_js($field_id) {
        ...
        $select_label = esc_js(__('Ödeme İkonu Seç', 'iyzico-payment-gateway'));
        $button_label = esc_js(__('Görsel Seç', 'iyzico-payment-gateway'));
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'enabled' => array(
                'title'   => __('Etkinleştir', 'iyzico-payment-gateway'),
                'type'    => 'checkbox',
                'label'   => __('iyzico ile ödemeyi etkinleştir', 'iyzico-payment-gateway'),
                'default' => 'no',
            ),
            'title' => array(
                'title'       => __('Başlık', 'iyzico-payment-gateway'),
                'type'        => 'text',
                'description' => __('Checkout sayfasında müşteriye görünen başlık.', 'iyzico-payment-gateway'),
                'default'     => __('Kredi/Banka Kartı', 'iyzico-payment-gateway'),
                'desc_tip'    => false,
            ),
            'description' => array(
                'title'       => __('Açıklama', 'iyzico-payment-gateway'),
                'type'        => 'textarea',
                'description' => __('Ödeme yöntemi seçildiğinde müşteriye görünen açıklama.', 'iyzico-payment-gateway'),
                'default'     => __('Kartınızla güvenli ödeme yapın. 3D Secure doğrulama sonrası siparişiniz onaylanır.', 'iyzico-payment-gateway'),
            ),
            'builtin_icon' => array(
                'title'       => __('Hazır İkon', 'iyzico-payment-gateway'),
                'type'        => 'select',
                'options'     => wic_builtin_icon_choices(),
                'default'     => '',
                'description' => __('Marka-bağımsız, plugin\'e gömülü basit bir ikon (SVG). Aşağıda bir görsel yüklersen bu ayar yok sayılır, yüklenen görsel öncelikli olur.', 'iyzico-payment-gateway'),
                'desc_tip'    => false,
            ),
            ...
            'sandbox' => array(
                'title'       => __('Sandbox modu', 'iyzico-payment-gateway'),
                'type'        => 'checkbox',
                'label'       => __('Test (sandbox) modunu etkinleştir', 'iyzico-payment-gateway'),
                'default'     => 'yes',
                'description' => __('Canlıya almadan önce mutlaka sandbox key\'leriyle uçtan uca test et.', 'iyzico-payment-gateway'),
            ),
            'test_api_key' => array(
                'title' => __('Test API Key', 'iyzico-payment-gateway'),
                'type'  => 'text',
            ),
            'test_secret_key' => array(
                'title' => __('Test Secret Key', 'iyzico-payment-gateway'),
                'type'  => 'password',
            ),
            'live_api_key' => array(
                'title' => __('Live API Key', 'iyzico-payment-gateway'),
                'type'  => 'text',
            ),
            'live_secret_key' => array(
                'title' => __('Live Secret Key', 'iyzico-payment-gateway'),
                'type'  => 'password',
            ),
            'debug' => array(
                'title'       => __('Debug log', 'iyzico-payment-gateway'),
                'type'        => 'checkbox',
                'label'       => __('İşlem loglarını WooCommerce > Status > Logs altına yaz', 'iyzico-payment-gateway'),
                'default'     => 'yes',
            ),
            'identity_title' => array(
                'title'       => __('TCKN (Kimlik No) Toplama', 'iyzico-payment-gateway'),
                'type'        => 'title',
                'description' => __('iyzico API\'si her ödemede bir TCKN alanı istiyor; WooCommerce checkout\'u bunu varsayılan olarak toplamaz. Bu alanı açarsan checkout\'a bir "TC Kimlik No" alanı eklenir — zorunlu tutmazsan müşteri boş bırakabilir. Alan kapalıyken, ya da açık olup boş bırakıldığında, Türkiye\'de yaygın kullanılan standart doldurma değeri (11111111111) otomatik gönderilir. Not: bu alan, checkout\'ta hangi ödeme yönteminin seçileceğinden bağımsız olarak eklenir (WooCommerce ödeme yöntemi seçilmeden önce checkout alanlarını oluşturur), yani açarsan tüm siparişlerde görünür.', 'iyzico-payment-gateway'),
            ),
            'identity_field_enabled' => array(
                'title'       => __('TCKN Alanını Göster', 'iyzico-payment-gateway'),
                'type'        => 'checkbox',
                'label'       => __('Checkout\'a TC Kimlik No alanı ekle', 'iyzico-payment-gateway'),
                'default'     => 'no',
                'description' => __('Kapalıyken checkout\'ta hiçbir şey değişmez, iyzico\'ya otomatik olarak 11111111111 gönderilir.', 'iyzico-payment-gateway'),
            ),
            'identity_field_required' => array(
                'title'       => __('TCKN Girişini Zorunlu Kıl', 'iyzico-payment-gateway'),
                'type'        => 'checkbox',
                'label'       => __('Müşteri TCKN girmeden siparişi tamamlayamasın', 'iyzico-payment-gateway'),
                'default'     => 'no',
                'description' => __('Yalnızca yukarıdaki alan açıkken etkilidir. Kapalı bırakılırsa alan görünür ama boş geçilebilir; boş geçilirse yine 11111111111 gönderilir.', 'iyzico-payment-gateway'),
            ),
            'iyzico_panel_info' => array(
                /* translators: section title shown above the callback URL / IP fields */
                'title'       => __('iyzico Paneline Eklenecek Bilgiler', 'iyzico-payment-gateway'),
                'type'        => 'title',
                'description' => __('Bu değerleri iyzico panelinde Ayarlar > IP/Back URL Yönetimi sayfasına ekleyip onaya göndermen gerekiyor. Aşağıdaki alanlar düzenlenebilir — otomatik tespit edilen değer yanlışsa (ör. yönlendirme yapan bir domain kullanıyorsan) elle düzeltebilirsin, plugin gerçekten burada yazan değeri kullanır.', 'iyzico-payment-gateway'),
            ),
            'callback_url_override' => array(
                'title'       => __('Callback URL', 'iyzico-payment-gateway'),
                'type'        => 'text',
                /* translators: %s: computed default callback URL */
                'description' => sprintf(__('iyzico panelinde Back URL olarak bunu kaydet. Boş bırakılırsa siteni home_url() adresinden otomatik hesaplanır (%s).', 'iyzico-payment-gateway'), home_url('/api/payment/callback')),
                'placeholder' => home_url('/api/payment/callback'),
                'desc_tip'    => false,
            ),
            'server_ip' => array(
                'title'       => __('Sunucu Çıkış IP Adresi', 'iyzico-payment-gateway'),
                'type'        => 'wic_ip_detect',
                'description' => __('iyzico panelinde IP Adresleri listesine bunu ekle. "Tespit Et" butonu sunucunun iyzico\'ya giden isteklerde kullandığı gerçek çıkış IP\'sini bulur (ziyaretçi IP\'si değil).', 'iyzico-payment-gateway'),
            ),
            'reference_links' => array(
                'title' => __('Faydalı Linkler', 'iyzico-payment-gateway'),
                'type'  => 'wic_reference_links',
            ),
            'branding_title' => array(
                'title'       => __('Görünüm', 'iyzico-payment-gateway'),
                'type'        => 'title',
                'description' => __('Checkout\'ta ödeme kutusunun rengini kendi marka renklerine uyarlayabilirsin. Hiçbir şey değiştirmezsen mevcut yeşil palet kalır.', 'iyzico-payment-gateway'),
            ),
            'color_primary' => array(
                'title'       => __('Ana Renk', 'iyzico-payment-gateway'),
                'type'        => 'color',
                'default'     => '#2F453A',
                'description' => __('Güven rozeti ikonları ve metinleri bu rengi kullanır.', 'iyzico-payment-gateway'),
                'desc_tip'    => true,
            ),
            'color_bg' => array(
                'title'       => __('Kutu Arka Plan Rengi', 'iyzico-payment-gateway'),
                'type'        => 'color',
                'default'     => '#F5FAF6',
                'description' => __('Ödeme yöntemi seçildiğinde açılan kutunun arka planı.', 'iyzico-payment-gateway'),
                'desc_tip'    => true,
            ),
            'color_border' => array(
                'title'       => __('Kenarlık Rengi', 'iyzico-payment-gateway'),
                'type'        => 'color',
                'default'     => '#D7E7DC',
                'description' => __('Kutunun ince kenarlığı.', 'iyzico-payment-gateway'),
                'desc_tip'    => true,
            ),
            'health_check_title' => array(
                'title'       => __('Sağlık Kontrolü', 'iyzico-payment-gateway'),
                'type'        => 'title',
                /* translators: %s: site admin email address */
                'description' => sprintf(__('Callback endpoint erişilebilir mi, iyzico\'ya ağ bağlantısı var mı, API key/secret dolu mu — bugüne kadar elle yaptığımız kontrolleri otomatikleştiriyor. Günde bir kez arka planda da çalışır; bir sorun tespit edilir (veya düzelir) ise %s adresine otomatik e-posta gider.', 'iyzico-payment-gateway'), esc_html(get_option('admin_email'))),
            ),
            'health_check' => array(
                'title' => __('Durum', 'iyzico-payment-gateway'),
    ...
    public function generate_wic_health_check_html($key, $data) {
        ...
        <tr valign="top">
            ...
            <td class="forminp">
                <button type="button" class="button" id="wic-health-check-btn" data-nonce="<?php echo esc_attr(wp_create_nonce('wic_health_check')); ?>"><?php esc_html_e('Şimdi Kontrol Et', 'iyzico-payment-gateway'); ?></button>
        ...
        (function() {
            ...
            var i18n = <?php echo wp_json_encode(array(
                'checking'    => __('Kontrol ediliyor (birkaç saniye sürebilir)...', 'iyzico-payment-gateway'),
                'done'        => __('Tamamlandı.', 'iyzico-payment-gateway'),
                'failed'      => __('Kontrol başarısız oldu.', 'iyzico-payment-gateway'),
                'lastCheck'   => __('Son kontrol:', 'iyzico-payment-gateway'),
                'statusOk'    => __('OK', 'iyzico-payment-gateway'),
                'statusWarn'  => __('UYARI', 'iyzico-payment-gateway'),
                'statusError' => __('HATA', 'iyzico-payment-gateway'),
            )); ?>;
        ...
        <tr valign="top">
            <td colspan="2" style="padding:0 0 20px;">
                <div style="background:#F5FAF6;border:1px solid #D7E7DC;border-radius:10px;padding:16px 20px;">
                    <p style="margin-top:0;"><strong><?php esc_html_e('Başlarken', 'iyzico-payment-gateway'); ?></strong></p>
                    <ol style="margin:0 0 8px;padding-left:20px;">
                        <li><?php esc_html_e('Sandbox modunda başla — Test API Key/Secret\'ı iyzico\'nun sandbox panelinden (sandbox-merchant.iyzipay.com) al, aşağıdaki ilgili alanlara gir.', 'iyzico-payment-gateway'); ?></li>
                        <li><?php esc_html_e('Aşağıdaki "Callback URL" ve "Sunucu Çıkış IP Adresi" değerlerini iyzico panelinde Ayarlar > IP/Back URL Yönetimi\'ne ekleyip onaya gönder.', 'iyzico-payment-gateway'); ?></li>
                        <li><?php esc_html_e('iyzico\'nun sandbox test kartlarından biriyle bir sipariş tamamla.', 'iyzico-payment-gateway'); ?></li>
                        <li><?php esc_html_e('En alttaki Sağlık Kontrolü bölümünden "Şimdi Kontrol Et"e bas — hepsi yeşil olmalı.', 'iyzico-payment-gateway'); ?></li>
                        <li><?php esc_html_e('Onay tamamlanınca: Sandbox modunu kapat, Live key\'leri gir, küçük tutarlı bir gerçek işlemle son kez test et.', 'iyzico-payment-gateway'); ?></li>
                    </ol>
                    <p style="margin:0;font-size:12.5px;color:#5A7666;">
                        <?php
                        printf(
                            /* translators: %s: link to the plugin's GitHub README */
                            esc_html__('Daha fazla ayrıntı için: %s', 'iyzico-payment-gateway'),
    ...
    public function generate_wic_ip_detect_html($key, $data) {
        ...
        <tr valign="top">
            ...
            <td class="forminp">
                ...
                <button type="button" class="button" id="wic-detect-ip-btn" data-target="<?php echo esc_attr($field_key); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce('wic_detect_ip')); ?>"><?php esc_html_e('Tespit Et', 'iyzico-payment-gateway'); ?></button>
        ...
        (function() {
            ...
            var i18n = <?php echo wp_json_encode(array(
                'detecting'  => __('Tespit ediliyor...', 'iyzico-payment-gateway'),
                'found'      => __('Bulundu — kaydetmeyi unutma.', 'iyzico-payment-gateway'),
                'notFound'   => __('Tespit edilemedi, elle gir.', 'iyzico-payment-gateway'),
            )); ?>;
    ...
    public function process_refund($order_id, $amount = null, $reason = '') {
        ...
        if (!$order) {
            return new \WP_Error('wic_refund_no_order', __('Sipariş bulunamadı.', 'iyzico-payment-gateway'));
        }
        ...
        if (empty($payment_transaction_id)) {
            ...
            return new \WP_Error(
                ...
                __('Bu sipariş için iyzico işlem numarası bulunamadı. Sipariş bu plugin\'in eski bir sürümüyle mi tamamlanmıştı? İadeyi iyzico Merchant Panel üzerinden manuel yapman gerekebilir.', 'iyzico-payment-gateway')
            );
        }
        ...
        if ((float) $amount !== (float) $order->get_total()) {
            return new \WP_Error(
                ...
                __('Bu sürüm kısmi iadeyi desteklemiyor — sadece siparişin tam tutarını iade edebilirsin.', 'iyzico-payment-gateway')
            );
        }
        ...
        if ('success' !== $refund->getStatus()) {
            $error = $refund->getErrorMessage() ?: __('İade işlemi iyzico tarafından reddedildi.', 'iyzico-payment-gateway');
        ...
        $order->add_order_note(sprintf(
            ...
            __('iyzico üzerinden %1$s iade edildi. %2$s', 'iyzico-payment-gateway'),
            wc_price($amount),
            $reason ? '(' . $reason . ')' : ''
        ));
    ...
    public function generate_wic_reference_links_html($key, $data) {
        ...
        $panel_label = $sandbox
            ? __('Sandbox Panel', 'iyzico-payment-gateway')
            : __('Live Panel', 'iyzico-payment-gateway');
        ...
        <tr valign="top">
            ...
            <td class="forminp">
                ...
                &middot;
                <a href="https://docs.iyzico.com" target="_blank" rel="noopener"><?php esc_html_e('Dokümantasyon', 'iyzico-payment-gateway'); ?></a>
                <p class="description">
                    <?php esc_html_e('Test kartı numaraları dokümantasyon sitesindeki "Test Kartları" bölümünde — güncel liste zaman zaman değişebildiği için buraya sabit yazmadık.', 'iyzico-payment-gateway'); ?>
                </p>
            </td>
        </tr>
        <?php
    ...
    public function add_identity_billing_field($fields) {
        $fields['billing_tckn'] = array(
            'label'             => __('TC Kimlik No', 'iyzico-payment-gateway'),
            'type'              => 'text',
    ...
    public function validate_identity_field() {
        ...
        if ('' === $value) {
            wc_add_notice(__('Lütfen TC Kimlik No alanını doldurun.', 'iyzico-payment-gateway'), 'error');
            return;
        }

        if (!self::is_valid_identity_format($value)) {
            wc_add_notice(__('TC Kimlik No 11 haneli, 0 ile başlamayan bir sayı olmalı.', 'iyzico-payment-gateway'), 'error');
        }
    }
    ...
    public function process_payment($order_id) {
        $order = wc_get_order($order_id);
        if (!$order) {
            wc_add_notice(__('Sipariş bulunamadı.', 'iyzico-payment-gateway'), 'error');
            return array('result' => 'failure');
        }

        if (empty($this->api_key) || empty($this->secret_key)) {
            $this->log('API key/secret ayarlanmamış, ödeme başlatılamadı.', 'error');
            wc_add_notice(__('Ödeme sistemi şu anda yapılandırılamamış durumda, lütfen bizimle iletişime geçin.', 'iyzico-payment-gateway'), 'error');
            return array('result' => 'failure');
        }
        ...
        $buyer->setName($order->get_billing_first_name() ?: __('Musteri', 'iyzico-payment-gateway'));
        ...
        $buyer->setRegistrationAddress($order->get_billing_address_1() ?: __('Adres belirtilmedi', 'iyzico-payment-gateway'));
        ...
        $basketItem->setName(sprintf(__('Siparis #%s', 'iyzico-payment-gateway'), $order->get_order_number()));
        $basketItem->setCategory1(__('Genel', 'iyzico-payment-gateway'));
        ...
        try {
            ...
            wc_add_notice(__('Ödeme başlatılırken bir hata oluştu, lütfen tekrar deneyin.', 'iyzico-payment-gateway'), 'error');
            return array('result' => 'failure');
        }
        ...
        if ('success' !== $checkoutFormInitialize->getStatus()) {
            ...
            wc_add_notice(sprintf(__('Ödeme başlatılamadı: %s', 'iyzico-payment-gateway'), $checkoutFormInitialize->getErrorMessage()), 'error');
            return array('result' => 'failure');
        }
        ...
        $order->add_order_note(sprintf(__('iyzico ödeme sayfasına yönlendirildi. Token: %s', 'iyzico-payment-gateway'), $checkoutFormInitialize->getToken()));
        $order->update_status('pending', __('iyzico ödemesi bekleniyor', 'iyzico-payment-gateway'));
        $order->save();
